...
export async function integrateData(
    ...
    for (let i = 0; i < outputQuestions.length; i++) {
      ...
      try {
        ...
        if (
          ...
          // APIキーが無効・未設定の場合は、以降のLLMマッチングをスキップ
          break;
        }
  ...
  for (let i = 0; i < outputQuestions.length; i++) {
    ...
    if (current.answerMethod === '運用者コメ' && current.liveJudgment === 'TRUE') {
      const currentHasTime = current.time && current.time.trim() !== '';
      ...
      for (let j = i - 1; j >= 0; j--) {
        ...
        const prevHasTime = prev.time && prev.time.trim() !== '';

        if (!currentHasTime || !prevHasTime) {
          // どちらかの時間が空白の場合は時間による打ち切り判定をせず、類似度判定を行う
