...
- **ファイルアップロード**: CSV/Excel形式のコメントピックアップシートファイルとQA抽出テキストファイルをアップロード
...
1. **コメントピックアップシートファイルのアップロード**: CSVまたはExcel形式の質問ピックアップ表をアップロード
...
### コメントピックアップシート（CSV/Excel）

- **ヘッダー行**: 「回答済」「Time」「User」「質問」「回答方法」を含む行を自動検出する（タイトル行の有無・列順・列数の変更に対応）
- **ヘッダー行より後**: データ行
- 上記5項目以外の列（配信媒体・コメント補足・回答・回答ご提案・キーワード抽出（自動）等）は取り込み時に無視される

### 出力Excel

「質問回答まとめ」シートに以下7列で出力する:

最終回答状況 / 配信現場判定 / アーカイブ判定 / 回答方法 / Time / User / 質問
